...
class AddressBook:
    ...
    def ajouter_contact(self, nom, email, telephone):
        contacts_existants = self.get_all_contacts()
        for c in contacts_existants:
            if c.email.lower() == email.strip().lower():
                print(f" Un contact avec l'email '{email}' existe déjà.")
                return
            if c.telephone == telephone.strip():
                print(f" Un contact avec le téléphone '{telephone}' existe déjà.")
                return
    ...
    def supprimer_contact(self, identifiant):
        ...
        contact_trouve = None
        for c in contacts:
            if c.email.lower() == identifiant.strip().lower() or c.telephone == identifiant.strip():
                contact_trouve = c
                break
 
        if contact_trouve is None:
            print(f" Aucun contact trouvé avec l'email ou le téléphone '{identifiant}'.")
            return
